...
info.iconset = array -- Optional XPlore iconset to be used
...
function UIDropDownFork_CreateFrames(level, index)
	...
	while ( level > UIDROPDOWNFORK_MAXLEVELS ) do
		...
		local templates = "UIDropDownForkListTemplate,BackdropTemplate"
		if XP and XP.IsClassicWOTLK then templates = "UIDropDownForkListTemplate" end
		local newList = CreateFrame("Button", "DropDownForkList"..UIDROPDOWNFORK_MAXLEVELS, nil, templates);
...
function UIDropDownFork_AddButton(info, level)
	...
	if ( info.notCheckable and not info.indented ) then
		if ( info.justifyH and info.justifyH == "CENTER" ) then
			...
			if ( info.icon or info.mouseOverIcon ) and not info.iconOnly then -- XP fix, move icon to the left, and text to right of icon
...
function ToggleDropDownFork(level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button, autoHideDelay)
	...
	if ( listFrame:IsShown() and (UIDROPDOWNFORK_OPEN_MENU == tempFrame) ) then
		...
		XP:ScheduleTimer(function() listFrame.HideOnClick=true end,0)
...
function UIDropDownFork_ApplySkin()
	local SkinData = XP.UI.SkinData

	local checkbox_file = XP.ButtonSets.Interactions.file
	local arrow_file = XP.ButtonSets.TitleButtons.file
	local radio_coords_on = XP.ButtonSets.Interactions.RADIO_ON.texcoords
	local radio_coords_off = XP.ButtonSets.Interactions.RADIO.texcoords
	local checkbox_coords_on = XP.ButtonSets.Interactions.CHECKBOX_ON.texcoords
	local checkbox_coords_off = XP.ButtonSets.Interactions.CHECKBOX.texcoords
	local arrow_coords = XP.ButtonSets.TitleButtons.MORETABS.texcoords
	...
	for i=1,UIDROPDOWNFORK_MAXLEVELS do
		...
		for b=1,UIDROPDOWNFORK_MAXBUTTONS do
			...
			if txt and txt.SetFont then txt:SetFont(XP.Font,XP.db.profile.fontsize) end
			...
			XP.ButtonSets.TitleButtons.MORETABS:AssignToButton(arrow)
			arrow:GetNormalTexture():SetVertexColor(unpack(SkinData("Accent")))

			local icon = _G[buttonName.."Icon"]
			if button.isSeparator then
				icon:SetTexture(XP.SKINSDIR.."white");
			...
			highlight:SetTexture(XP.SKINSDIR.."white");
