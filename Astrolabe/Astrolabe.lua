...
-- [[ AstrolabeMapMonitor Embedding ]]
do
	local LIBRARY_VERSION_MAJOR = "AstrolabeMapMonitor"
	local LIBRARY_VERSION_MINOR = 99 -- !X-Libs

	if not DongleStub then error(LIBRARY_VERSION_MAJOR .. " requires DongleStub.") end
	if DongleStub:IsNewerVersion(LIBRARY_VERSION_MAJOR, LIBRARY_VERSION_MINOR) then
		...
		local function argcheck(value, num, ...)
			assert(1, type(num) == "number", "Bad argument #2 to 'argcheck' (number expected)")
			local n = (arg and arg.n) or select("#", ...)
			for i=1, n do
				if type(value) == select(i, ...) then return end
			end
			local name = string.match(debugstack(2,2,0), ": in function [`<](.-)['>]")
			error(string.format("Bad argument #%d to 'AstrolabeMapMonitor.%s' (got %s)", num, name or "?", type(value)), 3)
		end

		function AstrolabeMapMonitor:MonitorWorldMap( worldMapFrame )
			argcheck(worldMapFrame, 2, "table");
			if not worldMapFrame.SetParent then return end
			local TrackedWorldMaps = self.TrackedWorldMaps;
			if ( TrackedWorldMaps[worldMapFrame] ) then return 1; end
		...
		function AstrolabeMapMonitor:Update()
			...
			if ( (self.NumVisibleWorldMaps) > 0 ) then visibleMap = true; end
		...
		local function activate( newInstance, oldInstance )
			if ( oldInstance ) then
				for k, v in pairs(oldInstance) do
					if ( type(v) ~= "function" ) then newInstance[k] = v; end
				end
...
local LIBRARY_VERSION_MINOR = 999 -- !X-Libs
