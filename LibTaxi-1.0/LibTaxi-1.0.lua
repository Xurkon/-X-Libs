...
do
	...
	-- Initialize localization. All taxis are stored as ENGLISH - lookups need pre-translation.
		if Lib.TaxiNames_Local then
			...
			local mt = { __index = function(t,k)  local v=rawget(t,k)  if not v then print("|cffff4400Taxi |cffff8800"..k.."|r not translated to "..GetLocale()..", please report this to XPlore") return k elseif v==true then return k else return v end  end }
	...
	function Lib:Startup(newsave)
		...
		Lib.ZGLOGO = "|T"..XP.DIR.."\\Skins\\zglogo_single:8:8|t"
	...
	function Lib:GetMapContinent(mapID)
		local cont = XP.GetMapContinent(mapID)
		...
		if mapID==2405 then cont_scan=2479 end -- voidstorm
		if mapID==2413 then cont_scan=2480 end -- harandar
	...
	function Lib:GetTaxiTripTime(node1,node2)
		...
		local dist = XP.MapCoords.Mdist(node1n.m,node1n.x,node1n.y,node2n.m,node2n.x,node2n.y) or 0
	...
	function Lib:PlayerOnOffTaxi(ontaxi)
		...
		if ontaxi then
			if Lib.LastTaxi and (not taxis or not taxis[1] or taxis[1][2]>=50) then 
				local lastnodeid = Lib.LastTaxi.route and Lib.LastTaxi.route[1] and Lib.LastTaxi.route[1].nodeID
				if lastnodeid then
			...
			if XP.DEV or XP.db.profile.flash_on_taxi_arrival then FlashClientIcon() end
			...
			if taxis and taxis[1] and taxis[1][2]<50 then
				...
				XP.db.global.taxitimes = XP.db.global.taxitimes or {}
				...
				if not fc_array[dep_ident].neighbors or not fc_array[dep_ident].neighbors[arr_ident] then
					...
					XP.db.global.taxitimes[dep_ident.."__"..arr_ident]=triptime
					return
				end
				...
				XP.db.global.taxitimes = XP.db.global.taxitimes or {}
				XP.db.global.taxitimes[dep_ident.."_"..arr_ident]=triptime
				local reversed
				...
				--	XP.db.global.taxitimes[arr_ident.."_"..dep_ident]=triptime
				--end
	...
	function Lib.OnEvent(this, event, arg1,arg2)
		...
		elseif event == "TAXIMAP_OPENED" then
			...
			if XP.DEV then Lib.HooksForFlightMapFrame:DEV_HighlightIncompletePins() end

			if XP then XP:ShowWaypoints() end -- Force arrow to re-check itself -- Jeremiah
		...
		elseif event == "UI_INFO_MESSAGE" then
			if arg2==ERR_NEWTAXIPATH then
				Lib:LearnCurrentTaxi()
				if XP and XP.LibRover then XP.LibRover:UpdateNow("quiet") end -- Try to force update of arrow ~~ Jeremiah
			end
	...
	function Lib:InitializeTaxis()
		if not Lib.taxipoints then
			Lib:Debug("No taxipoints data loaded - skipping taxi initialization")
			return
		end
			...
			for z,zone in pairs(cont) do
				...
				while n<=#zone do
					...
					if node.faction~=enemyfac then
						...
						n=n+1
					elseif enemyfac ~= "DON'T PRUNE" then
						tremove(zone,n)
					else
						n=n+1
					end
	...
	function Lib:FindTaxi(name,trim)
		...
		for c,z,n,node in XP.CoroPairs(Lib.taxipoints,IterateCZNN) do
 	...
 	function Lib:MarkKnownTaxis()
		...
		for c,cont in pairs(Lib.taxipoints) do
			for z,zone in pairs(cont) do
				...
				zoneid=XP and XP.Pointer:SanitizePhase(zoneid)
				...
				for n,node in ipairs(zone) do
					...
					elseif node.questKnown then
						self:LearnTaxi(node,IsQuestFlaggedCompleted(node.questKnown))
					...
					elseif false and not Lib.master[c] then  -- we didn't scan this continent yet, so let's do some guessing
						...
						--[[ if LibRover.data.ZoneContLev[zoneid].level<=level  -- zone is lower level than player, we should know all taxis by now
						  ...
						  then
							if  (node.quest and not XP.completedQuests[node.quest]) -- we didn't do the quest
	...
	function Lib:ScanTaxiMap()
		...
		if XP.Pointer.tmp_taxis_assumed then LibRover:UpdateNow() end
		...
		if XP.db.profile.taxi_print_unknowntimes then
	...
	function Lib:CacheRoutesAsNodeIDs()
		...
		if not self.lastCurrentSlot then return end

		local taxis,taxisbyslots = self:GetBlizNodesByEverything()
		local currentSlot = taxisbyslots[self.lastCurrentSlot]
		if not currentSlot then return end

		self.lastCurrentNodeID = currentSlot.nodeID
		if not self.lastCurrentNodeID  then return end

		for endSlot=1,NumTaxiNodes() do
			if TaxiNodeGetType(endSlot)~="DISTANT" then
				local endNode = taxisbyslots[endSlot]
				if not endNode then return end
				local endNodeID = endNode.nodeID
	...
	function Lib:GetTotalFlightTimeTo(destNodeID)
		...
		for i=1,#route-1 do
			...
			if not t then 
				self:Debug(("No trip time! %s %s %s."):format(tostring(err),route[i],route[i+1]))
				return false
			end
	...
	function Lib:MergeData()
		...
		for c,z,ni,node in XP.CoroPairs(Lib.taxipoints,IterateCZNN) do
			local found
		...
		for c,cont in pairs(Lib.taxipoints) do
			for z,zone in pairs(cont) do
				for ni,node in pairs(zone) do
					if not is_enemy(node.faction,playerF) then
						if not node.taxinodeID then 
							tinsert(Lib.errors,"Why did "..node.name.." not get a nodeID?") 
						else
							local fcdata = Lib.fc_by_nodeID[node.taxinodeID]
							if not fcdata then 
								tinsert(Lib.errors,"taxi "..node.name.." id "..node.taxinodeID.." has no fcdata?") 
	...
	function Lib:ImportTaxiTimesFromSV()
		if not XP.db.global.taxitimes then return end
		local loaded=0
		for k,v in pairs(XP.db.global.taxitimes) do  if v>0 then
		...
		--XP:Print(("DEV: %d taxi trip times are still in SVs, waiting to be saved!"):format(loaded))
	end

	function Lib:GetTaxiByTarget(token)
		local id = XP.GetUnitId(token or "target")
	...
	function Lib:HookTaxiMapPins_Delayed()
		...
		--if XP.IsClassic or XP.IsClassicWOTLK then return end
		-- dev function disabled, broken on classic
		--if not XP.DEV then return end
		--if XP.db.profile.taxi_print_unknowntimes and not 
		--if not self.taxi_tooltip_timer then self.taxi_tooltip_timer = XP:ScheduleRepeatingTimer(function() self:FlightMapFrameTimer() end,0.1) end
	end
		...
		hooksecurefunc("TakeTaxiNode",function(destSlot,a,b)
			...
			if destData and destData.nodeID then
				...
				if Lib.LastTaxi.node then
					Lib.LastTaxi.name,Lib.LastTaxi.zone = Lib.LastTaxi.node.name,XP.GetMapNameByID(Lib.LastTaxi.node.m)
					--Lib.LastTaxi.name,Lib.LastTaxi.zone = Lib.LastTaxi.node.name,(Lib.LastTaxi.node.m and XP.GetMapNameByID(Lib.LastTaxi.node.m) or ("(bad map in '"..Lib.LastTaxi.name.."')")) -- just in case? or was it a bad update that broke in here?
				else
	...
	function Lib:AnyTaxiKnown(...)
		local count = select("#", ...)
		for i = 1, count do
			local nodeid = select(i, ...)
			local node = Lib.fnode_by_nodeID[nodeid]
			if node and node.known then return true end
		end
		return false
	end
	...
	function Lib:Debug(s,...)
		if XP and XP.db then XP:Debug("&LibTaxi "..s,...) end
	end
	...
	function Lib:DEV_DumpFlightCosts(onlycont)
		...
		for contnum,contdata in XP.OrderedPairs(Lib.flightcost) do  repeat
			if onlycont and contnum~=onlycont then break end  --continue
			if XP.Testing.FlightCostComments[contnum] then
				s=s.."\n\n\n\n\n"..XP.Testing.FlightCostComments[contnum]
			end
			...
			local function namesort(a,b) 
				if a.name==b.name then
					if a.nodeID and b.nodeID then
						return a.nodeID < b.nodeID
					else
						return (a.faction or "")< (b.faction or "")
					end
				else
			...
			for i,data in ipairs(contdata) do
				...
				if data.neighbors then
					...
					table.sort(sorted_neighs,XP.NumericSafeSort)
			...
			s=s.."}\n"
		until true end
		XP:ShowDump(s,"Paste into LibTaxi/data.lua"..(onlycont and " WHERE APPROPRIATE" or ""))
	end

	function Lib:DEV_ScanCurrentTaxiNPC()
		XP.db.global.scannedtaxinpcs = XP.db.global.scannedtaxinpcs or {}
		local saved_npcs = XP.db.global.scannedtaxinpcs
		-- prune known saved first
		for c,z,n,node in XP.CoroPairs(Lib.taxipoints,IterateCZNN) do  if node.npcid then saved_npcs[node.npcid]=nil end  end

		if self:GetTaxiByTarget("npc") then return end -- already known, by target npc ID
	...
	function Lib:DEV_CheckNodeMatches()
		...
		if #errors>0 then XP:ShowDump(table.concat(errors,"\n"),"LibTaxi problems") end
	end
	...
	function Lib:DEV_ViewTaxiMapData()  -- DEV FUNCTION (simple)
		...
		if Spoo then
			...
			XP:ShowDump(table.concat(ret,"\n"),"Taxi data for continent "..cont)
		end
	end

	local XPloredevhooked
	function Lib:DEV_HookXPloreButtons()
		if not (XP and XP.DEV) then return end
		...
		if not XPloredevhooked then
			...
			XPloredevhooked=true
		end

		taxiparent.XPloreButtonScan = taxiparent.XPloreButtonScan or XP.ChainCall(CreateFrame("BUTTON","ZGVLibTaxiButton_in_"..taxiparent:GetName(),taxiparent,"UIPanelButtonTemplate"))
		...
		taxiparent.XPloreButtonDetails = taxiparent.XPloreButtonDetails or XP.ChainCall(CreateFrame("BUTTON","ZGVLibTaxiButton_in_"..taxiparent:GetName(),taxiparent,"UIPanelButtonTemplate"))
		...
		taxiparent.XPloreButtonNPC = taxiparent.XPloreButtonNPC or XP.ChainCall(CreateFrame("BUTTON","ZGVLibTaxiButton_in_"..taxiparent:GetName(),taxiparent,"UIPanelButtonTemplate"))
		...
		:Hide()
		.__END
		local count=0  for id,data in pairs(XP.db.global.scannedtaxinpcs or {}) do count=count+1 end
		taxiparent.XPloreButtonNPC:SetText(("[Z] Dump NPCs (%d)"):format(count))

		if XP.db.profile.debug_display then
			taxiparent.XPloreButtonScan:Show()
			taxiparent.XPloreButtonDetails:Show()
			taxiparent.XPloreButtonNPC:Show()
		end
			...
			for contnum,contdata in ipairs(LibRover.data.flightcost) do
				for tag,taxidata in pairs(contdata) do
					...
					if not taxinode then
						if XP.DEV then print("WHOA. Taxi node not found: ",taxidata.Name) end   -- DEVed prints at 2014-11-12
					else
	...
	--[[ UNUSED? not even for DEV!
		function Lib:DumpNeighborFlights()
			...
			s=s.."	},\n},"
			XP:ShowDump(s,"Paste these into LibTaxi data.lua into data.flightcost")
		end
	--]]
	...
	if XP.DEV then
		...
		for hop = 1, GetNumRoutes(mouseSlot) do
			...
			if not isPrecise and not destinationPin.taxiNodeData.isMapLayerTransition then
				if XP.db.profile.taxi_print_unknowntimes then print((hop==1 and "|cffffdd00" or "|cffff0000").."Unknown time ["..hop.."]: "..startPin.taxiNodeData.name.." to "..destinationPin.taxiNodeData.name) end
...
function HooksForFlightMapFrame:DEV_HighlightIncompletePins()
	local provider = self:GetFlightPathDataProvider() 
	local pins = provider and provider.slotIndexToPin
...
---@return Button?
function HooksForTaxiFrame:GetCurrentTaxiMapButton() --Vanilla
		...
		if route then
			for hop=1,#route-1 do
				...
				if not isPrecise then
					...
					if XP.db.profile.taxi_print_unknowntimes then print((hop==1 and "|cffffdd00" or "|cffff0000").."Unknown time ["..hop.."]: "..startNode.name.." to "..endNode.name) end
				end
			end
		end
