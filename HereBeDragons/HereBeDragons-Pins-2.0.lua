...
local MAJOR, MINOR = "HereBeDragons-Pins-2.0", 99
assert(LibStub, MAJOR .. " requires LibStub")

-- Universal Lua 5.0+ shims for Retail APIs
local g = getfenv(0)
if not g.CreateFromMixins then
	g.CreateFromMixins = function(...)
		local m = {}
		local args = {...}
		local n = (args.n) or (table and table.getn and table.getn(args)) or 0
		if n == 0 and args[1] then
			for i, _ in pairs(args) do
				if type(i) == "number" and i > n then n = i end
			end
		end
		for i = 1, n do
			local obj = args[i]
...
if not g.MapCanvasDataProviderMixin then g.MapCanvasDataProviderMixin = {} end
if not g.MapCanvasPinMixin then g.MapCanvasPinMixin = {} end

-- Shim WorldMapFrame:GetCanvas() and pinPools for 3.3.5a
if _G.WorldMapFrame then
    if not _G.WorldMapFrame.GetCanvas then
        _G.WorldMapFrame.GetCanvas = function(self)
            return _G.WorldMapButton or _G.WorldMapDetailFrame or self
        end
    end
    if not _G.WorldMapFrame.pinPools then
        _G.WorldMapFrame.pinPools = {}
    end
end
...
-- Instance remaps for special scenarios
local instanceRemaps = {
    [1616] = 1220, -- Azsuna Mage Hunter scenario
}

--- Get the direction to a minimap icon
-- @param icon The minimap icon frame
-- @return angle, distance in yards, or nil if icon not found or wrong instance
function pins:GetDirectionToIcon(icon)
    ...
    local remap = instanceRemaps[instanceID]
    if remap then instanceID = remap end
    if instanceID ~= data.instanceID then return nil, nil, nil end

    return HBD:GetWorldVector(instanceID, oX, oY, data.x, data.y)
end

--- Get the distance to a minimap icon
-- @param icon The minimap icon frame
-- @return distance in yards, deltaX, deltaY, or nil if icon not found or wrong instance
function pins:GetDistanceToIcon(icon)
    local data = minimapPins[icon]
    if not data then return nil, nil, nil, nil end

    local oX, oY, instanceID = HBD:GetPlayerWorldPosition()
    if not oX then return nil, nil, nil, nil end

    local remap = instanceRemaps[instanceID]
    if remap then instanceID = remap end
    if instanceID ~= data.instanceID then return nil, nil, nil, nil end

    return HBD:GetWorldDistance(instanceID, oX, oY, data.x, data.y)
end
