...
do
	local path = string.match(debugstack(1, 1, 0), "AddOns/(.+)LibGraph%-2%.0%.lua") or string.match(debugstack(1, 1, 0), "AddOns\\(.+)LibGraph%-2%.0%.lua")
	if path then
		TextureDirectory = "Interface\\AddOns\\"..path
	else
		path = string.match(debugstack(1, 1, 0), "([^/\\]+)\\Libs\\LibGraph%-2%.0") or string.match(debugstack(1, 1, 0), "([^/\\]+)/Libs/LibGraph%-2%.0")
		if path then
			TextureDirectory = "Interface\\AddOns\\"..path.."\\Libs\\LibGraph-2.0\\"
		else
			TextureDirectory = "Interface\\AddOns\\X-Plore_WOTLK335\\Libs\\LibGraph-2.0\\"
		end
	end
end
...
local function SetupGraphRealtimeFunctions(graph, upgrade)
	...
	graph.SetUseHotspots = GraphFunctions.SetUseHotspots
	graph.SetUseHotlines = GraphFunctions.SetUseHotlines
	graph.RegisterHotspot = GraphFunctions.RegisterHotspot

	graph.SetDrawGrid = GraphFunctions.SetDrawGrid
	...
	graph.SetYLabelsFormatter = GraphFunctions.SetYLabelsFormatter
	graph.SetXLabelsFormatter = GraphFunctions.SetXLabelsFormatter
...
function GraphFunctions:SetDrawGrid(state)
	self.DrawGrid = state
end
...
function GraphFunctions:SetXLabels(Top, Bottom)
	self.XLabelTop = Top
	self.XLabelBottom = Bottom
end

function GraphFunctions:SetYLabelsFormatter(formatter)
	self.YLabelsFormatter = formatter
end

function GraphFunctions:SetXLabelsFormatter(formatter)
	self.XLabelsFormatter = formatter
end
...
function GraphFunctions:CreateGridlines()
	...
	if self.DrawGrid and self.YGridInterval then
			...
			if i ~= 0 or not self.YAxisDrawn then
				...
				if ((i ~= UpperYGridLine) or (TopSpace > 12)) and (NoSecondary or math_fmod(i, self.GridSecondaryY) == 0) then
					if self.YLabelsLeft then
						...
						if self.YLabelsFormatter then
							F:SetText(self.YLabelsFormatter(i * self.YGridInterval))
						else
	...
	if self.DrawGrid and self.XGridInterval then
		...
		for i = LowerXGridLine, UpperXGridLine do
			if i ~= 0 or not self.XAxisDrawn then
				...
				if self.XLabelBottom then
					F = self:FindFontString()
					F:SetFontObject("GameFontHighlightSmall")
					F:SetTextColor(1, 1, 1)
					F:ClearAllPoints()
					F:SetPoint("TOP", T, "BOTTOM", 0, 0)
					if self.XLabelsFormatter then
						F:SetText(self.XLabelsFormatter(i * self.XGridInterval))
					else
				...
				if self.XLabelTop then
					F = self:FindFontString()
					F:SetFontObject("GameFontHighlightSmall")
					F:SetTextColor(1, 1, 1)
					F:ClearAllPoints()
					F:SetPoint("BOTTOM", T, "TOP", 0, 0)
					if self.XLabelsFormatter then
						F:SetText(self.XLabelsFormatter(i * self.XGridInterval))
					else
						F:SetText(i * self.XGridInterval)
...
function GraphFunctions:OnUpdateGraph()
	if self.UseHotspots or self.UseHotlines then
		self.HotspotHighlight:Hide()

		if (MouseIsOver(self)) then
			local sX, sY = self:GetLeft(),self:GetBottom()
			local Scale = self:GetEffectiveScale()
			local mX, mY = GetCursorPosition()
			local dX = mX / Scale - sX
			local dY = -(sY - mY / Scale)

			GameTooltip:Hide()

			for i,point in ipairs(self.Hotspots) do
				if (not self.UseHotspots or (point[1]<dY and dY<point[2])) and point[3]<dX and dX<point[4] then
					self.HotspotHighlight:SetPoint("BOTTOMLEFT",self,"BOTTOMLEFT",point[3]-5,point[1]-5)
					self.HotspotHighlight:Show()
					GameTooltip:SetOwner(self,"ANCHOR_CURSOR")
					GameTooltip:SetText(point[5])
					GameTooltip:Show()
					break
				end
			end
...
function GraphFunctions:SetUseHotspots(state)
	self.UseHotspots = state

	if state and not self.HotspotHighlight then
		self.HotspotHighlight = self:CreateTexture(nil, "ARTWORK")
		self.HotspotHighlight:SetTexture(XP.DIR .. "\\Skins\\door-highlight-neg")
		self.HotspotHighlight:SetSize(20,20)
	end
end

function GraphFunctions:SetUseHotlines(state)
	self.UseHotlines = state
...
function GraphFunctions:RegisterHotspot(x,y,text)
	x = floor(x)
	y = floor(y)
	-- top,bottom,left,right
	table.insert(self.Hotspots,{y-5,y+5,x-5,x+5,text})
end
...
function GraphFunctions:RefreshLineGraph()
	...
	for k1, series in pairs(self.Data) do
		...
		for k2, point in pairs(series.Points) do
			...
			if self.UseHotspots or self.UseHotlines then
				self:RegisterHotspot(LastPoint.x,LastPoint.y,point[3] or point[2])
			end
		end
	end
