...
data.basenodes.FloorCrossings = --[[LIBROVER]] {
	...
	-- MIDNIGHT
		["Voidstorm"] = {
			"Voidstorm/0 47.92,78.75 <radius:30> -x- Voidlands Abundance/0 65.15,13.51 <radius:15>"..
				"{title_atob:Follow the path down\nand enter the cave} {title_btoa:Follow the path up\nand leave the cave}",		-- Voidlands Abundance Cave
		},

		["Eversong Woods M"] = {
			"Eversong Woods M/0 56.76,65.79 <radius:10> -x- Eversong Woods M/1 88.60,34.75 <radius:10>"..
				"{title_atob:Enter the building} {title_btoa:Leave the building}",		-- Watha'nan Crypts entrance
			"Eversong Woods M/1 43.98,84.98 <radius:10> -x- Eversong Woods M/0 54.76,68.05 <radius:10>"..
				"{title_atob:Leave the building} {title_btoa:Enter the building}",		-- Watha'nan Crypts entrance
		},
	
	-- TWW
		["Lorewalking Ny'alotha"] = {
			"Lorewalking Ny'alotha/1 49.36,54.60 <radius:10> -x- Lorewalking Ny'alotha/0 57.72,23.29 <radius:10> {title_atob:Enter the cave} {title_btoa:Leave the cave}",
			"Lorewalking Ny'alotha/1 60.16,53.47 <radius:10> -x- Lorewalking Ny'alotha/2 32.57,46.17 <radius:10> {title:Use the Path of the Chosen ability}",
			"Lorewalking Ny'alotha/1 45.39,43.48 <radius:10> -x- Lorewalking Ny'alotha/3 78.13,88.53 <radius:10> {title_atob:Enter the building} {title_btoa:Leave the building}",
			"Lorewalking Ny'alotha/1 52.55,78.71 <radius:10> -x- Lorewalking Ny'alotha/4 51.02,12.68 <radius:10> {title_atob:Enter the building} {title_btoa:Leave the building}",
		},
	...
	-- SHADOWLANDS
		...
		["Oribos"] = {
			-- The Idyllia, Oribos -x- Downstairs, Oribos --
				"Oribos/0 69.00,41.44 <radius:15> -x- Oribos/2 57.83,27.61 <radius:15>",
			-- The Idyllia, Oribos -x- Downstairs, Oribos --
				"Oribos/0 70.50,59.94 <radius:15> -x- Oribos/2 55.63,71.15 <radius:15>",
		},
	...
	-- Pandaria
		...
		["Krasarang Wilds"] = {
			"Krasarang Wilds/0 80.30,17.71 <radius:7> -x- Krasarang Wilds/1 33.37,22.27 <radius:7> {title_atob:Enter the cave} {title_btoa:Leave the cave}",
			"Krasarang Wilds/1 66.47,49.74 <radius:7> -x- Krasarang Wilds/3 64.21,37.28 <radius:12> {title_atob:Run down the stairs} {title_btoa:Run up the stairs}",
			"Krasarang Wilds/3 89.85,48.42 <radius:7> -x- Krasarang Wilds/2 77.14,41.93 <radius:7> {title_atob:Enter the side cave} {title_btoa:Leave the side cave}",
		},
		["Kun-Lai Summit"] = {
	...
	-- LEGION
		...
		["Dalaran L"] = {
			-- Underbelly Entrance for Warlock Order Hall Portal
				"Dalaran L/10 34.33,45.60 <radius:10> -x- Dalaran L/11 19.18,57.14 <radius:10>"..
					...
					"{title_btoa:Run up the stairs and leave the Underbelly} {cond:XP.IsLegionOn()}",
		...
		["Val'sharah"] = {					
			-- The Dreamgrove Ground Entrance (Druid Only)
				"Val'sharah/0 45.48,34.51 <radius:20> -x- The Dreamgrove/0 44.82,32.76 <radius:20>"..
					...
					"{cond:select(2,UnitClass('player'))=='DRUID' and XP.IsLegionOn()}",
		},
		...
		["Highmountain"] = {
			-- Trueshot Lodge Ground Entrance (Hunter Only)
				"Highmountain/0 36.13,44.71 <radius:20> -x- Trueshot Lodge/0 42.72,9.98 <radius:20>"..
					...
					"{cond:select(2,UnitClass('player'))=='HUNTER' and XP.IsLegionOn()}",
		},
		...
		["Stormheim"] = {
			...
			-- Vault of Eyir
				"Stormheim/0 62.82,68.11 <radius:15> -x- Stormheim/28 26.43,51.00 <radius:15> {template:building}",
		},		
		...
		["Suramar"] = {
			-- Elor'shan Cave
				"Suramar/0 65.95,42.06 <radius:15> -x- Suramar/34 56.43,86.96 <radius:15> {template:cave}",
				
			-- Kel'balor Cave
				"Suramar/0 59.36,43.03 <radius:15> -x- Suramar/35 54.39,86.69 <radius:15> {template:cave}",
				
			-- Teloth'aran Cave
				"Suramar/0 54.63,46.35 <radius:15> -x- Suramar/22 39.35,82.60 <radius:15> {template:cave}",
		},
