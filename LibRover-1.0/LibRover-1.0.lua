...
local IsSpellKnown,UnitLevel,IsQuestFlaggedCompleted,GetSpellCooldown
    = IsSpellKnown,UnitLevel,(_G.IsQuestFlaggedCompleted or C_QuestLog.IsQuestFlaggedCompleted),C_Spell.GetSpellCooldown or GetSpellCooldown
local yield,resume,co_create,co_status = coroutine.yield,coroutine.resume,coroutine.create,coroutine.status

local XP

-- WOW_PROJECT constants may not exist when this library first loads
...
local WOW_PROJECT_ID = _G.WOW_PROJECT_ID or WOW_PROJECT_WRATH_CLASSIC
...
local TIMERUNNING_SEASON_PANDARIA = (Constants and Constants.TimerunningConsts and Constants.TimerunningConsts.TIMERUNNING_SEASON_PANDARIA) or 1
local TIMERUNNING_SEASON_LEGION = (Constants and Constants.TimerunningConsts and Constants.TimerunningConsts.TIMERUNNING_SEASON_LEGION) or 2
...
Lib.data = Lib.data or (addon and addon.LibRoverData)
	if addon then addon.LibRoverData = nil end
...
local function getdist(node1,node2)
	if not Mdist then Mdist=XP.MapCoords.Mdist end
...
local function playerpos()
	local m,f=XP.CurrentMapID,XP.CurrentMapFloor -- WHY OH WHY do I have to do it like that ;_;
...
function Lib:UpdateConfig()
	...
	if travelmode == 1 then
		if Lib.extradata then Lib.extradata.direct=true end
...
local function MapName(id,floor,usename)
	...
	if usename then
		local mapname = XP.GetMapNameByID(tonumber(id) or 0,floor or 0) or "(map "..tostring(id).."?)"
		local floorname = XP.GetMapFloorNameByID(tonumber(id) or 0,floor or 0)
		if mapname==floorname then return mapname else return floorname end  -- mapname.." - "..
	else
		return XP.GetMapNameByID(tonumber(id) or 0,floor or 0) or "(map "..tostring(id).."?)"
	end
end
Lib.MapName=MapName
...
local function AddNode(node,dontlink)
	...
	node.c = node.c or XP.GetMapContinent(node.m)  --Lib.ContinentsByID[node.m]
	...
	--node.f = XP:SanitizeMapFloor(node.m,node.f) -- with bfa changes, we should already have sane uimapid here
...
local cond_env = setmetatable({  -- some fancy functions for node conditions to use
	...
	QuestActiveOrComplete = function(id) return XP.Parser.ConditionEnv.questactive(id) or C_QuestLog.IsQuestFlaggedCompleted(id) end,
	UnitLevel = function() return XP:GetPlayerPreciseLevel() end,
	PlayerLevel = function() return XP:GetPlayerPreciseLevel() end,
	covenant = function(covenant) return XP.Parser.ConditionEnv[covenant] end,
	covenantnetwork = function() return (XP.Parser.ConditionEnv.covenantfeature('Transport Network')) end,
	hasbuff = function(spell) return XP.Parser.ConditionEnv.hasbuff(spell) end,
},{__index=_G})

local function ParseDataCond(data)
	if type(data.cond)=="string" then
		local err
		data.cond_fun,err = XPLoadString("return "..data.cond, cond_env)
...
local function __SmartAddTextNodes(text,deftype,dontlink,extra1,extra2)
	local data
	...
	if n1 and extra1 then for k,v in pairs(extra1) do n1[k]=v end end
	if n2 and extra2 then for k,v in pairs(extra2) do n2[k]=v end end
	...
	--[[
	if n2 then
		Lib:Debug(("New smart node: %s %.1f,%.1f  %s  %s %.1f,%.1f"):format(XP.GetMapNameByID(m1),x1,y1, twoway and "x" or "to", XP.GetMapNameByID(m2),x2,y2))
	else
		Lib:Debug(("New smart node: %s %.1f,%.1f  %s  %s"):format(XP.GetMapNameByID(m1),x1,y1, twoway and "x" or "to", XP.GetMapNameByID(m2)))
	end
	--]]
...
local function __SmartAddArrayNodes(data,deftype,dontlink,extra1,extra2)
...
local function SmartAddNode(data,deftype,dontlink,extra1,extra2)
	-- all-purpose "map x,y x map x,y"
	local aok,n1,n2
	local DEV=true--XP.DEV

	if tostring(data):find("-- ",1,true) then
		if XP.db.profile.debug_display then tinsert(Lib.NODE_SOURCES,setmetatable({},{__tostring=function() return tostring(data):gsub("\n"," "):sub(1,100) end}))  end
		return
	end

	if type(data)=="string" then
		if DEV then aok,n1,n2 = pcall(__SmartAddTextNodes,data,deftype,dontlink,extra1,extra2)
		else n1,n2=__SmartAddTextNodes(data,deftype,dontlink,extra1,extra2)  aok=true  end
	...
	elseif type(data)=="table" then
		...
		elseif data[1]=="MAP" then
			...
			if DEV then aok,n1,n2 = pcall(__SmartAddArrayNodes,data,deftype,dontlink,extra1,extra2)
			else n1,n2=__SmartAddArrayNodes(data,deftype,dontlink,extra1,extra2)  aok=true  end
	...
	if n1 and n2 and n1.bordermeta then
		...
		if n1.bordermeta.title or n1.bordermeta.title_atob or n1.bordermeta.title_atob1 or n1.bordermeta.title_btoa or n1.bordermeta.title_bota1 then
	...
	if XP.db.profile.debug_display and n1 then
...
local function InitializeTaxis(dontlink)
	...
	for c,cont in XP.OrderedPairs(LibTaxi.taxipoints) do
		for z,zone in XP.OrderedPairs(cont) do
	...
	-- link taxis together
	for i,n1 in ipairs(XP.LibRover.nodes.taxi) do
		for j,n2 in ipairs(XP.LibRover.nodes.taxi) do
...
local function _StartupThread()
	...
	for i,v in pairs(Lib.data.point_context_templates) do v[2]=XP.Parser.apply_colours(v[2]) end
	...
	if XP.db.char.BakedCache then
		LibRover.data.neighbourhood=XP.db.char.BakedCache
	...
	if XP.db.profile.force_travel_cache then  Lib.data.version = Lib.data.version or {}  Lib.data.version.nodes_version=999  Lib.data.neighbourhood=Lib.data.neighbourhood or {}  Lib.data.neighbourhood.version=999  end
	...
	or XP.db.profile.travel_do_full_linking_at_startup
	then
		...
		use_cache=nil
		if XP.DEV then
			XP.Frame.Border:SetBackdropColor(1,0,0,1)
			XP.Frame.Border:SetBackdropBorderColor(1,0,0,1)
			XP:Print("|cffff0000LibRover says versions of data and cache don't match, or cache was disabled manually.\n|rIf you're working on map data, ignore this.")
		else
			XP:ErrorThrow("Error in travel system: map cache version mismatch. Cache is disabled. Please report this!")
		end
	...
	until XP.db.profile.pathfinding ---------==============  H A L T   H E R E  ================-----
	...
	do -- INITIALIZE WALLS
		local segment="walls"
		for zone,zdata in XP.OrderedPairs(Lib.data.walls or {}) do repeat
	...
	do -- INITIALIZE INNS
		...
		local progress=0
		for z,zone in XP.OrderedPairs(Lib.data.basenodes.inns) do
	...
	do -- INITIALIZE EXPLICIT FLOORING
		...
		local progress=0
		-- for id,zonedata in XP.OrderedPairs(Lib.data.basenodes.FloorCrossings) do --bfa alpha
		for id,zonedata in XP.OrderedPairs(Lib.data.basenodes.FloorCrossings) do
			progress=progress+1
	...
	do -- INITIALIZE INDOOR AREAS
		...
		local progress=0
		-- for id,data in XP.OrderedPairs(Lib.data.basenodes.indoorzones) do -- bfa alpha
		for id,data in XP.OrderedPairs(Lib.data.basenodes.indoorzones) do
			progress=progress+1
	...
	do -- INITIALIZE DUNGEON FLOORS
		...
		local progress=0
		for id,zonedata in XP.OrderedPairs(Lib.data.basenodes.DungeonFloors) do
			progress=progress+1
			for n,nodedata in ipairs(zonedata) do
				local node1,node2=SmartAddNode(nodedata,nil,use_cache)
				if node1 then node1.flooring=true end
	...
	if use_cache then
		...
		for worldflighttype,data in XP.OrderedPairs(Lib.data.neighbourhood) do  repeat
	...
	XP:Debug("&_WARN killing walks took "..t2-t1)
	--]]
	...
	do -- INITIALIZE PORTKEYS
		...
		for i,item in ipairs(Lib.data.portkeys) do repeat
			...
			if type(item.destination)=="string" and item.destination:sub(1,1)~="_" then
				...
				item.destination = SmartAddNode(item.destination,nil,nil,{onlyhardwire_to=true})
	...
	XP:SendMessage("LIBROVER_INITIALISED")
...
function Lib:StartupStep(timeleft)
	if not XP.loading_screen_disabled then return end
	...
	while thisframe<(timeleft or STARTUP_INTENSITY) do
		...
		--XP.Profiler:Start("travel-total")
		--XP.Profiler:Start("travel-tmp")
		local good,module,progress,quiet = resume(self.startup_thread)
		module=module or "?"
		--XP.Profiler:Stop("travel-tmp","travel-"..tostring(module))
		--XP.Profiler:Stop("travel-total")
		...
		if not good then
			...
			local errmsg = module
			self:StopStartup()
			XP:Print("ERROR initializing LibRover, check the Lua errors and report them, please.")
			error("\""..tostring(errmsg or "?").."\"\nin\n".. (stack or "(no stack)"))
		else
			if module==true then

				-- Summarize initialization steps.
				if XP.db.profile.debug then self:ShowStartupSummary() end
				...
				if self.find_after_load then
					...
					XP.Pointer.PathFoundHandler("failure")
					XP:ShowWaypoints()
				end
...
function Lib:DoStartup()
	XP = XPlore  -- That's local. Trimming down taint. Taint once and get over it.
	if not XP then return true end

	if not XP.db.profile.pathfinding then return true end

	STARTUP_INTENSITY = XP.STARTUP_INTENSITY or STARTUP_INTENSITY
	...
	self:UpdateConfig(XP.db.profile)
	if self.F then return end
	if not XP.LIBROVER_MANAGED_STARTUP then  -- XP will need to call LibRover:StartupStep(timeleft) to start it up.
		self.F=CreateFrame("FRAME")
		self.F:Show()
		self.F:SetScript("OnUpdate",function() Lib:StartupStep() end)
	else
		tinsert(XP.startups,{"Travel System",function()
...
function Lib:SetupInitialQuickTravel(startnode)
	...
	local function FindBindLocation(bind)
		...
		XP.FAILEDHEARTH = bind
		if XP.DEV and not Lib.FAILEDHEARTHNAME==bind then geterrorhandler()("Travel has no idea where player's hearthstone is bound to: "..bind) Lib.FAILEDHEARTHNAME=bind end
		return nil
	end
...
function Lib:QueueFindPath(am,ax,ay,bm,bx,by, handler, extradata, force_new, quiet)
	if self.FAKE_STARTING_POINT_SETUP then  -- don't queue it! store it as fake start.
		...
		XP:Print(("Fake starting point set at %s %d,%d."):format(XP.GetMapNameByID(bm),bx*100,by*100))
		return
	end
...
function Lib:UpdateNow(quiet,speed)
	...
	self:Debug("Updating route NOW. Quiet=%s, speed=%.1f.",tostring(quiet),speed or XP.db.profile.pathfinding_speed)
...
function Lib:IsDestinationImpossible(mymap,destmap)
	...
	local level = XP:GetPlayerPreciseLevel()
	...
	local cont = XP.GetMapContinent(destmap)
	...
	and not (XP.CurrentGuide and XP.CurrentGuide.monkquest and select(2,UnitClass("player"))=="MONK") then
	--Pandaria is level locked, but low level monks can get to peak during monk daily quests
		local title
		local questdata = XP.Localizers:GetQuestData(fac=="Alliance" and 29548 or 29690)
	...
	elseif destmap==1961 and not IsQuestFlaggedCompleted(63665) then
		local title
		local questdata = XP.Localizers:GetQuestData(63665)
		title = questdata and questdata.title
	...
	elseif (destmap==1670 or destmap==1671) and (not IsQuestFlaggedCompleted(60151) and not PlayerIsOnQuest(85028)) and level<50 then
		local title
		local questdata = XP.Localizers:GetQuestData(60151)
		title = questdata and questdata.title
	...
	elseif false and (-- Scenario maps, reachable only through LFG
		...
		then
		local title=destmap~=899 and XP.GetMapNameByID(destmap) or "Arena of Annilhilation" --TODO Localize Area of Annilhilation; GMNBID(899)=="Proving Grounds"...
...
function Lib:BoatLockDisable(mode)
	...
	if XP.db.profile.debug_librover_boat_timedprevent then
		if mode=="menu" then
...
function Lib:FindPath(am,ax,ay,bm,bx,by, handler, extradata, force_new, quiet)
	if not XP.db.profile.pathfinding then return false end
	...
	if am==0 then
		--local m,f=XP.CurrentMapID,XP.CurrentMapFloor
		...
		if x and y and x>0 and y>0 and x<1 and y<1 then
			am,af,ax,ay=m,f,x,y
			--Lib:Debug("We're actually in %s at %.1f %.1f", XP.GetMapNameByID(lam) or lam,lax*100,lay*100)
		end
		--]]
	end

	--af = af and XP:SanitizeMapFloor(am,af,"is_current") -- floors are already converted to maps and sane
	--bf = bf and XP:SanitizeMapFloor(bm,bf)
	...
	self:Debug("&_PUSH |cff00ff88FindPath|r:  |cffaaff88%s|r##%d |cffffffff%.1f|r,|cffffffff%.1f|r --> |cffaaff88%s|r##%d |cffffffff%.1f|r,|cffffffff%.1f|r, %s",
		am and XP.GetMapNameByID(am) or am or "nil",am or 0,ax and ax*100 or -1,ay and ay*100 or -1,
		bm and XP.GetMapNameByID(bm) or bm or "nil",bm or 0,bx and bx*100 or -1,by and by*100 or -1,
...
function Lib:InitializePath_Threaded() --called in as part of StepForever_Threaded only.
	...
	if Lib.debug_cnodes then XP.Pointer:ClearWaypoints("manual") end
...
function Lib:StepPath()  -- THE WORKHORSE.
	...
	--current.m = XP.Pointer:SanitizePhase(current.m)
	--current.f = XP:SanitizeMapFloor(current.m,current.f)
	...
	if #neighs==0 then
		...
		for no,nm in pairs(current.n) do
			...
			--no.m = XP.Pointer:SanitizePhase(no.m)
			--no.f = XP:SanitizeMapFloor(no.m,no.f)
			...
			if current==Lib.startnode and (no.flagtestresult or not no.testflags or no.steponflag) and no.setflags and current.m==no.m and current.f==no.f and getdist(current,no)<=(no.radius or 20) then
				...
				for flag,set in pairs(no.setflags) do
					...
					if old~=set and XP.db.profile.debug then
						ZoneTextString:SetText( "XPlore Nav OMG!" )
	...
	local cost_debugging = XP.db.profile.debug_display
	...
	for ni=1,#current_n do  local current_n_ni=current_n[ni]  local neigh,neighlink=current_n_ni[1],current_n_ni[2]
		...
		then
			...
			elseif mode == "teleport" or mode=="hearth" or mode=="astralrecall" or mode=="useitem" then
				...
				--if XP.db.profile.pathfinding_preferfly then mycost=mycost*0.3 end
			...
			if COST_MODIFIERS_BY_MODE[mode] then mytime = mytime * COST_MODIFIERS_BY_MODE[mode] end
			...
			-- unused!
				...
				if XP.db.profile.pathfinding_preferfly
			...
			if not neigh.cost or cost<neigh.cost then
				if lib_debug_onodes or (lib_debug_nodes_any and lib_debug_nodes[neigh.num]) then
					...
					--XP.Pointer:SetWaypoint(neigh.m,neigh.f,neigh.x,neigh.y,{title=neigh:tostring(true),overworld=true})
				end
				if debugged_pair then
	...
	if Lib.debug_cnodes or Lib.debug_openclose or (lib_debug_nodes_any and lib_debug_nodes[current.num]) then
		...
		--[[
		XP.Pointer:ClearWaypoints("manual")
		for ni,no in ipairs(self.nodes.all) do
			if no.status=="closed" then
				XP.Pointer:SetWaypoint(no.m,no.f,no.x,no.y,{title=no:tostring(true),overworld=true,type="manual"})
			end
		end
		--]]
		XP.Pointer:SetWaypoint(current.m,current.f,current.x,current.y,{title=current:tostring(true),overworld=true,type="manual"})
	end
...
function Lib:ReportPath(endnode)
	...
	-- PREPARE NODES FOR DISPLAY. Assign titles based on situation.
		for n=1,#results do repeat
			...
			local nextmap = XP.GetMapNameByID(nextnode and (nextnode.taxiDestination and nextnode.taxiDestination.m or nextnode.m) or 0)
			...
			local text = text
				...
				:gsub("{item}", XP:GetItemInfo(node.item or (node.link and node.link.item) or 0) or "item")
				:gsub("{arrivaltoy}", XP:GetItemInfo(node.arrivaltoy or (node.link and node.link.arrivaltoy) or 0) or "arrivaltoy")
				:gsub("{npc}",XP.Localizers:GetTranslatedNPC(node.npcid,node.npc) or "?")
			...
			if XP.db.profile.debug_display then node.maplabel = node.maplabel .. "\n" .. node:tostring() end
		until true end
	...
	if #results==2 and not results[2].noskip then
		if getdist(results[1],results[2])<XP.Pointer:GetDefaultStepDist() then return self:ReportArrival() end
	end
	...
	XP:SendMessage("LIBROVER_TRAVEL_REPORTED")
...
function Lib:OnUpdate(elapsed)
	...
	if self.calculating and self.thread then
		...
		--[[ "Try to use up 'leftover' time in a frame". Bullshit. There is NO leftover time.
			...
			time_slot = max(self.pathfinding_speed_override or XP.db.profile.pathfinding_speed or 0.001,free_time*1000)  -- in ms!!
			...
			self:Debug("FPS %.2f, msPF %.2fms, elapsed %.2f, free_time %.2fms, speed %.2fms, slot %.2fms",
				...
				elapsed*1000,
				free_time*1000,
				self.pathfinding_speed_override or XP.db.profile.pathfinding_speed or 0.001,
				time_slot)
		--]]

		time_slot = self.pathfinding_speed_override or XP.db.profile.pathfinding_speed or 1
		...
		self:Debug("&lr_calc &_REPLACE lr_calc_1 FPS %.2f, speed %.2fms, slot %.2fms",fps,self.pathfinding_speed_override or XP.db.profile.pathfinding_speed or 0.001,time_slot)
		...
		while time_slot_remaining>=0 and self.calculating do
			...
			if resumed then
				...
				XP:Error("Travel System crashed!\n"..(code or ""))
				self.calculating=false
				return
			end
		...
		--XP:Debug("&LibRover Ela %.2f, sec_per %.2f, Time slot: %.2f, taken: %.2f, covered %d steps", elapsed*1000,sec_per_frame*1000, time_slot,time_slot-time_slot_remaining,self.calculation_step)
	...
	elseif self.updating and not self.delayed_by_taxi and not InCombatLockdown() and lbm and not XP.db.profile.disable_travel_updates then
		...
		if lastupdate>self.update_interval then
			...
			if elapsed_for_update>UPDATE_FREQ then
				...
				if moved then
					...
					self.pathfinding_speed_override = XP.db.profile.pathfinding_speed/5
					self.quiet=true
					self.force_update_now=true
				end
			end
...
function Lib:Abort(whence,quiet)
	...
	if was_updating then XP.Pointer.PathFoundHandler("aborted") end
end
...
function Lib:CheckMaxSpeeds()
	...
	local Q_CYRCE_WOULD_BE_SO_PROUD = IsQuestFlaggedCompleted(85657)
	local MAP_SIRENISLE = 2369
	...
	if XP and XP.db then  -- debug overrides
		flyspeed=XP.db.profile.debug_librover_maxspeed or flyspeed
		if XP.db.profile.debug_librover_flightazeroth~=nil then canfly_in_azeroth=XP.db.profile.debug_librover_flightazeroth end
		if XP.db.profile.debug_librover_flightnorthrend~=nil then canfly_in_northrend=XP.db.profile.debug_librover_flightnorthrend end
		if XP.db.profile.debug_librover_flightpandaria~=nil then canfly_in_pandaria=XP.db.profile.debug_librover_flightpandaria end
		if XP.db.profile.debug_librover_flightdraenor~=nil then canfly_in_draenor=XP.db.profile.debug_librover_flightdraenor end
		if XP.db.profile.debug_librover_flightlegion~=nil then canfly_in_legion=XP.db.profile.debug_librover_flightlegion end
		if XP.db.profile.debug_librover_flightbfa~=nil then canfly_in_bfa=XP.db.profile.debug_librover_flightbfa end
		if XP.db.profile.debug_librover_flightsl~=nil then canfly_in_sl=XP.db.profile.debug_librover_flightsl end
		if XP.db.profile.debug_librover_dragonriding~=nil then 
			dragonspeed = XP.db.profile.debug_librover_dragonriding and 10
	...
	for zoneid=1,3000 do
		local system = XP.GetMapContinent(zoneid)
...
local function onEvent(this, event, arg1, arg2, arg3, arg4, arg5)
	--[[ -- startup moved to XP:OnInitialize
	if event=="ADDON_LOADED" and arg1==addonName then
		--Lib.ready=true  -- TODO added a XP.db check for sanity, test how it is working in real life
	...
	elseif event == "UNIT_ENTERING_VEHICLE"
		...
		and XP.GetCurrentMapID()==62 then -- fixed mapid for bfa
	...
	elseif event=="UNIT_SPELLCAST_SUCCEEDED" then
		...
		if Lib.DarkshoreCatSpells[arg3] then
			...
			if LibTaxi.LastTaxi.node then
				LibTaxi.LastTaxi.name,LibTaxi.LastTaxi.zone = LibTaxi.LastTaxi.node.name,XP.GetMapNameByID(LibTaxi.LastTaxi.node.m)
			else
	...
	elseif event=="ZONE_CHANGED" or event=="ZONE_CHANGED_NEW_AREA" or event=="ZONE_CHANGED_INDOORS" or event=="LOADING_SCREEN_DISABLED" then
		if event=="LOADING_SCREEN_DISABLED" then
			if XP.db.profile.debug_librover_boat_loading then
		...
		--Lib:RecordVindicaar() --use realtime map check now
		Lib:UpdateNow("quiet",XP.db.profile.pathfinding_speed/3) -- hard force update, but quiet
	...
	elseif (event=="UNIT_FLAGS" and arg1=="player") then
		...
		if old_ontaxi ~= ontaxi then
			old_ontaxi = ontaxi
			XP:SendMessage("XPlore_PLAYER_TAXI_CHANGED",ontaxi)
...
function Lib:ShowAllNodes(nodetype,do_spoo)
	...
	for ni,n in ipairs(Lib.nodes[nodetype]) do
		if n.m==m then
			...
			if do_spoo then tinsert(ret,n) else XP.Pointer:SetWaypoint(n.m,n.x,n.y,{title="|cff9999aa"..n:tostring(true):gsub("%(state","|n(state").."|r",qqtruesize=n.radius or 200,icon=XP.Pointer.Icons[icon_by_status[n.status or "nil"]],debugnodes=1,pathnode=n,overworld=true,}) end
		end
	end
...
function Lib:DEV_ShowAllWalls()
	...
	for wi,wall in ipairs(Lib.walls[mapid] or {}) do
		for si,seg in ipairs(wall.segments) do
			XP.Pointer:SetWaypoint(mapid,seg[1],seg[2],{title=("wall %d/%d/a %.2f,%.2f"):format(wi,si,seg[1]*100,seg[2]*100),icon=XP.Pointer.Icons.ant,debugnodes=1,drawlineid=mapid*10000+wi*100+si})
			XP.Pointer:SetWaypoint(mapid,seg[3],seg[4],{title=("wall %d/%d/b %.2f,%.2f"):format(wi,si,seg[3]*100,seg[4]*100),icon=XP.Pointer.Icons.ant,debugnodes=1,drawlineid=mapid*10000+wi*100+si})
		end
	end
end
...
function Lib.ShowDebugMenu(parent,but)
	...
	if subzones_src then
		for i,map in ipairs(subzones_src) do
			...
			local truename = XP.GetMapFloorNameByID(map.mapID)
			tinsert(current_level,{
				...
				onEnterFunc = function(self)
					...
					XP.ChainCall(Lib.debug_overlayrect)
						:ClearAllPoints()
	...
	tinsert(menu,{ text = "Show nodes and connections", notCheckable=true, hasArrow=true, menuList = {
		{ text = "Hardwired", notCheckable=true, func=function() LibRover:ShowAllNodes() LibRover:DEV_ShowAllWalls() XP.Pointer.debug_drawlines_walkfly=false end },
		{ text = "ALL", notCheckable=true, func=function() LibRover:ShowAllNodes() LibRover:DEV_ShowAllWalls() XP.Pointer.debug_drawlines_walkfly=true end },
	...
	tinsert(menu,{ text = "Subzones of '"..XP.GetMapFloorNameByID(mapid).."':",  notCheckable=true, disabled=not subzones_src, hasArrow=not not subzones_src, menuList = subzones_menu, keepShownOnClick=true })
...
function LibRover:SetupFakeStartingPoint()
	if self.FAKE_STARTING_POINT then
		...
		self.FAKE_STARTING_POINT_SETUP = false
		XP:Print("Fake starting point removed.")
	else
		self.FAKE_STARTING_POINT = nil
		self.FAKE_STARTING_POINT_SETUP = true
		XP:Print("Shift-click the map to set a fake starting point.")
	end
end
...
function LibRover:DebugPath(...)
	if not ... or (...=="clear") then
		...
		print("|cffeeddaaLibRover DebugPath|r set up:")
			...
			if not node then XP:Print(("No such node: %d"):format(ni)) break end
...
function LibRover:Explain()
	...
	local n1_info = XP.GetMapInfo(n1.m)
	local n2_info = XP.GetMapInfo(n2.m)
	...
	local n1parent_info = XP.GetMapInfo(n1_info.parentMapID) or {name="UNKNOWN", mapType=-999}
	local n2parent_info = XP.GetMapInfo(n2_info.parentMapID) or {name="UNKNOWN", mapType=-999}

	local n1continent = XP.GetMapContinent(n1.m)
	local n2continent = XP.GetMapContinent(n2.m)

	local n1continent_info = XP.GetMapInfo(n1continent) or {name="UNKNOWN", mapType=-999}
	local n2continent_info = XP.GetMapInfo(n2continent) or {name="UNKNOWN", mapType=-999}
	...
	local goal
	if XP.Pointer.current_waypoint and XP.Pointer.current_waypoint.goal then
		goal = XP.Pointer.current_waypoint.goal
	else
		if XP.CurrentStep then
			for i,v in pairs(XP.CurrentStep.goals) do
...
function LibRover:DumpAllNodes(zone)
	...
	XP:Timerize(
		function(self)
			...
			if zone then s = s .. ("(zone %s only)\n"):format(XP.XP.GetMapNameByID(zone)) end

			XP:Print ("Preparing a listing of Travel System map nodes, please wait...")
			yield()
			...
			for nn,node in ipairs(self.nodes.all) do  repeat
				...
				s = s .. ("%04d. %s /%d %02d,%02d (%s) %s"):format(nn,XP.GetMapNameByID(node.m),node.f,node.x*100,node.y*100,node.type, (node.type=="taxi" or node.type=="inn") and node.name  or  (node.id and "@"..node.id)  or  "")
				if node.border == self.nodes.all[nn+1] then  s = s .. " - - "  force_next=true  else  s = s .. "\n"  end
				if nn%100==0 then yield() end
			until true  end
			XP:ShowDump(s,"Travel System map nodes listing:")
		end,
	self)
end
...
function LibRover:FindNode(map,f,x,y)
	if type(map)=="string" and not (f or x or y) then
		map,f,x,y = XP.Parser.ParseMapXYDist(map)
	else
		x=x/100
		y=y/100
	end
...
function Lib:GetFloorByMapID(m)
	...
	if not mapdata then 
		...
		if mapdata then
			...
			XP:ShowDump("Warning, looking for map "..m.." that is not known. Temporary assigning "..mapdata.name..", it may change later. Please give following to devs:\n"..o)
...
function Lib:GetPlayerPosition()
	...
	local instance = (XP and XP.db and XP.db.char.fakeinstance) or select(8,GetInstanceInfo())
	...
	if pos then
		...
		if wx and XP and XP.HBD then x,y = XP.HBD:GetZoneCoordinatesFromWorld(wx,wy,map,false) end
	end
		...
		if ux and uy then
			for i=1,#remap[map] do  local remapdata=remap[map][i]
				if (not remapdata.cond or remapdata.cond()) and (remapdata.top>=uy and uy>=remapdata.bottom and remapdata.left>=ux and ux>=remapdata.right) then
					if XP and XP.HBD then
						x,y = XP.HBD:GetZoneCoordinatesFromWorld(ux,uy,remapdata.target,true)
					end
...
function Lib.ZoneIsOutdoor(mapid)
	local mapdata = XP.GetMapInfo(mapid)
...
do -- Tests
	...
	function LibRover.Tests.TestProto:Run(callback)
		...
		if self.settings.riding then XP.db.profile.debug_librover_maxspeed=self.settings.riding end
		if self.settings.flight then
			if self.settings.flight.Azeroth then XP.db.profile.debug_librover_flightazeroth=true end
			if self.settings.flight.Northrend then XP.db.profile.debug_librover_flightcold=true end
			if self.settings.flight.Pandaria then XP.db.profile.debug_librover_flightpandaria=true end
			if self.settings.flight.Draenor then XP.db.profile.debug_librover_flightdraenor=true end
			if self.settings.flight.Legion then XP.db.profile.debug_librover_flightlegion=true end
		end
...
do -- TaxiWhistle predictor
	...
	function TWP:SetupListener()
		...
		--if XP.DEV then XP:Print("|cffffee00 (DEV) Taxi Whistle|r predictor is ready.") end
	...
	function TWP:AnnouncePrediction()
		...
		print(("Expected Flight Master Whistle destinations from |cff88ffff%s/%d %d,%d|r:"):format(XP.GetMapNameByID(self.pm),self.pf,self.px*100,self.py*100))
		...
		for i=1,3 do
			...
			print(("%s%d. |cffff88ff%d|r yd - |cffffff88%s|r (|cff88ffff%s/%d %d,%d|r)"):format(i==1 and ">> " or "",i,dist,taxi.name,XP.GetMapNameByID(taxi.m),taxi.f,math.round(100*taxi.x),math.round(100*taxi.y),dist))
		end
		print("Any zone:")
		for i=1,3 do
			local taxi = self.taxidists[i][1]
			local dist = self.taxidists[i][2]
			print(("%d. |cffff88ff%d|r yd - |cffffff88%s|r (|cff88ffff%s/%d %d,%d|r)"):format(i,dist,taxi.name,XP.GetMapNameByID(taxi.m),taxi.f,math.round(100*taxi.x),math.round(100*taxi.y),dist))
		end
		...
		--print("(see XP.TaxiWhistle.last_taxi_list for a full list)")
	end
	...
	function TWP:ConfirmPrediction()
		...
		if taxidists[1] and taxidists[1][2]<50 then
			...
			if here_taxi==good_taxi then
				...
				print(("|cffff8888Bummer.|r This is |cffffff88%s|r (|cff88ffff%s/%d %d,%d|r) which was at |cffff88ff%.1f|r yd from departure..."):format(here_taxi.name,XP.GetMapNameByID(here_taxi.m),here_taxi.f,math.round(100*here_taxi.x),math.round(100*here_taxi.y),was_dist))
			end
		end
	end
...
do -- Arcantina handler
	...
	function Arcantina:Init()
		...
		if XP.db.char.keyarcantina and Arcantina.ExitDestination then
			Arcantina.ExitDestination:Update(unpack(XP.db.char.keyarcantina))
		end
	end
...
do -- Player Housing handler
	...
	function Housing:Init()
		...
		if XP.db.char.houseexit and Housing.ExitDestination then
			Housing.ExitDestination:Update(unpack(XP.db.char.houseexit))
		end
		
		Housing.HomeNode = Housing:GetHomeNode()
		if XP.db.char.housespot and Housing.HomeNode then
			Housing.HomeNode:Update(unpack(XP.db.char.housespot))
		end
	...
	function Housing:UpdateHomePosition()
		...
		for i,v in pairs(locations) do
			if v.ownerType == Enum.HousingPlotOwnerType.Self and v.ownerName==playerName then
				XP.db.char.housespot = {C_Map.GetBestMapForUnit("player"),v.mapPosition.x,v.mapPosition.y}
				Housing.HomeNode:Update(unpack(XP.db.char.housespot))
				break
			end
		end
	...
	function Housing:Arriving()
		...
		XP.db.char.houseexit = {m,x,y}
	end

	function Housing:Teleport()
		local neighborhoodGUID = XP.Modules.Housing.House.neighborhoodGUID
		local houseGUID = XP.Modules.Housing.House.houseGUID
		local plotID = XP.Modules.Housing.House.plotID
		return {
			["type"]="teleporthome",
	...
	function Housing:CanArrive(faction)
		...
		if not XP.Modules.Housing.House then return false end

		if faction=="Horde" then 
			return XP.GetCurrentMapID() ~= 2351
		end
		if faction=="Alliance" then 
			return XP.GetCurrentMapID() ~= 2352
		end
		return false
	end
	...
	function Housing:CanDepart()
		...
		return (XP.GetCurrentMapID() == 2351 or XP.GetCurrentMapID() == 2352)
	end
...
do -- Mole Machine handler
	...
	function MMH:SetupListener()
		if select(2,UnitRace("player"))~="DarkIronDwarf" and not XP.db.profile.debug_fake_darkiron then return end
	...
	function MMH.FrameOnEvent(frame,event,arg1,arg2,arg3)
		if event=="UNIT_SPELLCAST_SUCCEEDED" then
			...
			if (arg3==265225 --[[D.I.Mole Machine]] or arg3==168657 --[[bubble wand toy]]) then
				XP:Debug("Mole Machine appeared!")
				local x,y,m = LibRover:GetPlayerPosition()
			...
			elseif (arg3>=280906 and arg3<=280924) or arg3==280928 then
				MMH.last_mole = nil
			--[[
			elseif XP.DEV and MMH.last_mole and MMH.last_mole.time and time()-MMH.last_mole.time<30 then
				local spellname = getSpellName(arg3)
				if spellname and spellname:find("Mole") then
					XP:Error("Mole machine arrival spell: "..arg3)
				end
			--]]
			end
		end
...
function Lib:ValidDHSMap()
	return not bad_dhs_maps[XP.CurrentMapID]
end
