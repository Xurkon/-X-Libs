-- GLOBAL DEBUG_DOLINKAGE,LibRover,LibRover_Node,XPlore
...
function LibRover_Node:DoLinkage(n2,dryrun)
	...
	if D_DL then XP:Debug("Before fly in %.3fms",debugprofilestop()-_t) _t=debugprofilestop() end

	canfly,reasonfly = n1:CanFlyTo(n2,dryrun)

	if D_DL then XP:Debug("Before walk in %.3fms",debugprofilestop()-_t) _t=debugprofilestop() end
	...
	if D_DL then XP:Debug("Before neigh in %.3fms",debugprofilestop()-_t) _t=debugprofilestop() end
	...
	if D_DL then XP:Debug("Done in %.3fms",debugprofilestop()-_t) _t=debugprofilestop() end
...
function LibRover_Node:GetText(prevnode,nextnode,dir)
	...
	if Lib.debug_verbose_nodes then
		return ("[%d] %s %d %d,%d (%s)"):format(self.num,XP.GetMapNameByID(self.m or 0) or "world",self.m,self.x*100,self.y*100,self.type)
	...
	elseif self.type=="border" and (self.border or self.ms) then
		--if fromnode and fromnode.n[node] and fromnode.n[node].border then
			...
			--return ("%s border"):format(XP.GetMapNameByID(fromnode:GetText()) or dstmap)
		--else
			--return ("%s/%s border"):format(XP.GetMapNameByID(self.border[1]) or self.border[1],XP.GetMapNameByID(self.border[2]) or self.border[2])
		--end
	...
	elseif self.type=="portal" then
		...
		--if not srcmap then
			--return ("%s/%s portal"):format(XP.GetMapNameByID(self.n[fromnode]border[1]),XP.GetMapNameByID(self.border[2]))
		--else
		--	local dstmap = self.border[1]==srcmap and dstmap or srcmap
		--	return ("Portal %s %s"):format(dir=="src" and "to" or "from",XP.GetMapNameByID(dstmap))
		--end
...
function LibRover_Node:IsTaxiKnown()
	...
	elseif (self.factionid and XP:GetReputation(self.factionid).standing<(self.factionstanding or 3)) then return false,"|cffff0000unavailable (faction rep)|r",false
	...
	elseif (self.level and XP:GetPlayerPreciseLevel()<self.level) or (not C_Map.GetMapLevels(self.m) or C_Map.GetMapLevels(self.m)>XP:GetPlayerPreciseLevel()+5) then return false,"|cffff0000unavailable (high lvl)|r",false
...
local MAPENUM_UNDERCITY_CLASSIC=1458
...
local MAPENUM_IRONFORGE_CLASSIC=1455
...
local MAPENUM_VALE_OLD1 = 390
local MAPENUM_VALE_OLD2 = 395
local MAPENUM_VALE_OLD3 = 396
local MAPENUM_VALE_OLD={[MAPENUM_VALE_OLD1]=1,[MAPENUM_VALE_OLD2]=1,[MAPENUM_VALE_OLD3]=1}
local MAPENUM_VALE_NEW = 1530
...
function LibRover_Node:CanFlyTo(dest,debug)
	...
	if (m==MAPENUM_VALE_NEW or dest_m==MAPENUM_VALE_NEW) and XP.InPhase("oldvale") then return false,debug and "no flying between timetravel zones" end
	if (MAPENUM_VALE_OLD[dest_m] or MAPENUM_VALE_OLD[m]) and not XP.InPhase("oldvale") then return false,debug and "no flying between timetravel zones" end
	
	--exodar is handled by flyable=false
	--tol barad is handled by flyable=false
	if selfmeta.vashjir~=destmeta.vashjir then return false,debug and "either is vash" end
	
	if (m==MAPENUM_IRONFORGE)~=(dest_m==MAPENUM_IRONFORGE) then return false,debug and "no flying in/out of Ironforge" end
	if (m==MAPENUM_IRONFORGE_CLASSIC)~=(dest_m==MAPENUM_IRONFORGE_CLASSIC) then return false,debug and "no flying in/out of Ironforge" end
	...
	if (m==MAPENUM_UNDERCITY_CLASSIC and dest_m~=MAPENUM_UNDERCITY_CLASSIC) or (m~=MAPENUM_UNDERCITY_CLASSIC and dest_m==MAPENUM_UNDERCITY_CLASSIC) then return false,debug and "'no flying' in Undercity" end
	...
	local mapinfo_src = XP.GetMapInfo(m)
	local mapinfo_dst = XP.GetMapInfo(dest_m)
...
function LibRover_Node:InterfaceWithLib(lib)
	...
	XP=XPlore
	ZGV_Pointer_phasedMaps=	XP.Pointer.phasedMaps
