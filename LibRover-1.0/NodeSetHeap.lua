-- GLOBAL LibRover_NodeSetHeap,XP
...
function NodeSet:Add(node)
	...
	--if self.indices[node] then XP:Error("HEAP CRASHED! Node "..node.num.." added twice!") return end
...
function NodeSet:BubbleUp(node)
	...
	if not ind then
		XP:Error("HEAP CRASHED! No index in heap for node ".. node.num .." = " ..tostring(node) .. "? Calculations might be crazy.")
		return
		--[[
...
function NodeSet:Clear()
	...
	self.indices={}
	self.count=0
	--XP:Debug("&LibRover &LRadv NodeSetHeap clear")
end
