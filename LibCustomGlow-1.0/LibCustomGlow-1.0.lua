...
-- Universal Shims for 3.3.5a
local CreateTexturePool = _G.CreateTexturePool
...
if not CreateTexturePool or not CreateFramePool then
    local function Pool_Acquire(self)
        ...
        local new = false
        if not obj then
            obj = self.factory(self.parent, self.template)
            new = true
        end
        if self.resetter then self.resetter(self, obj) end
    ...
    local function CreatePool(factory, parent, template, resetter)
        return {
            active = {},
            inactive = {},
            factory = factory,
    ...
    CreateTexturePool = CreateTexturePool or function(parent, layer, sublayer, resetter)
        return CreatePool(function(p) return p:CreateTexture(nil, layer) end, parent, nil, resetter)
    end

    CreateFramePool = CreateFramePool or function(frameType, parent, template, resetter)
        return CreatePool(function(p, t) return CreateFrame(frameType, nil, p, t) end, parent, template, resetter)
    end
end
local ceil, floor, min, mod = math.ceil, math.floor, math.min, mod
local tinsert, tremove = table.insert, table.remove
