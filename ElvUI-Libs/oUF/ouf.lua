...
if(global) then
	...
	elseif(_G[global]) then
		-- oUF: Error: ElvUI is setting its global to an existing name "ElvUF"
		-- Skip registration if already exists to avoid collision errors when multiple versions are loaded.
	else
		_G[global] = oUF
	end
end
